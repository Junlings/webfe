...
def name_to_rgb_float(name, spec='css3'):
    """
    Convert a color name to a 3-tuple of percentages suitable for use
    in an ``rgb()`` triplet specifying that color.
    ...
    """
    strtuple = rgb_to_rgb_percent(name_to_rgb(name, spec=spec))
    st = []
    for stri in strtuple:
        st.append(float(stri.split('%')[0])/100.0)
    return st
...
if __name__ == '__main__':
    ...
    colornamelist = get_color_list()
    
    for colorname in colornamelist:
        print colorname,name_to_rgb_float(colorname)
