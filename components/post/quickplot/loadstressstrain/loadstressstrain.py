from loadstressstrain_xrc import xrcLSSFRAME
import wx
import random
from wx.lib.pubsub import setuparg1
from wx.lib.pubsub import pub

class lssframe(xrcLSSFRAME):
    def __init__(self,parent):
        xrcLSSFRAME.__init__(self,parent)
    ...
    def POST_LD_QUICKPLOT_Submit(self,event):
        ...
        # =========process stress/strain command
        itemtype = 'Element Scalar'
        selected1 = self.POST_D_ItemSource1.GetStringSelection()
        selected2 = self.POST_D_ItemSource2.GetStringSelection()
        selected = selected2 + ' of ' + selected1
        ...
        option2 = 0 # inner nodeid 
        dcommand = "%s,%s,%s,%s,%s,%s" % (dtablename,itemtype,selected,option2,target,nincstr)
        ...
        if self.POST_D_ItemSelSingle.GetValue() == True:
            com += dtablename + ':Elem_%s-%s' % (target,option2)
        else:
